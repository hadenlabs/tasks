# Prompt: Actualizar Taskfile template de terraform

## Contexto

Estas trabajando dentro de este repositorio. Debes seguir estrictamente todas las reglas definidas en `AGENTS.md`.

Este repo es una coleccion de templates `Taskfile.yml` (go-task) + automatizacion del proyecto. Debes mantener el template en `src/terraform/Taskfile.yml` y su documentacion de uso remoto.
...
Actualizar y estandarizar el template de Taskfile para la herramienta: `terraform`
...
- Estilo Taskfile:
  - Usar `version: "3"`.
  - Usar `---` al inicio cuando aplique; para `src/terraform/Taskfile.yml`, incluyelo.
...
1. Crear o actualizar `src/terraform/Taskfile.yml`
   - Debe seguir patrones de templates existentes.
   - Incluir como minimo:
     - `check` (orquesta dependencias de checks)
     - `check:terraform` (o `check:<binary>` real) con `preconditions: command -v ...`
   - Incluir tareas adicionales solo si existe un equivalente claro para terraform.
...
3. Documentar el include remoto en `docs/usage.md`
   ...
   - Si agregaste `TERRAFORM_VERSION` (u otras vars), reflejarlo tambien en el bloque `vars:` del ejemplo (solo si aplica).
...
1. Implementacion completa de `src/terraform/Taskfile.yml`
