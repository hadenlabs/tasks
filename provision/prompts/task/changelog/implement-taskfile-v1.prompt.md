# Prompt: Actualizar Taskfile template de changelog

## Contexto

Estas trabajando dentro de este repositorio. Debes seguir estrictamente todas las reglas definidas en `AGENTS.md`.

Este repo es una coleccion de templates `Taskfile.yml` (go-task) + automatizacion del proyecto. Debes mantener el template en `src/changelog/Taskfile.yml` y su documentacion de uso remoto.
...
Actualizar y estandarizar el template de Taskfile para la herramienta: `changelog`
...
- Estilo Taskfile:
  - Usar `version: "3"`.
  - Usar `---` al inicio cuando aplique; para `src/changelog/Taskfile.yml`, incluyelo.
...
1. Crear o actualizar `src/changelog/Taskfile.yml`
   - Debe seguir patrones de templates existentes.
   - Incluir como minimo:
     - `check` (orquesta dependencias de checks)
     - `check:changelog` (o `check:<binary>` real) con `preconditions: command -v ...`
...
3. Documentar el include remoto en `docs/usage.md`
   ...
   - Si agregaste `CHANGELOG_VERSION` (u otras vars), reflejarlo tambien en el bloque `vars:` del ejemplo (solo si aplica).
...
1. Implementacion completa de `src/changelog/Taskfile.yml`
