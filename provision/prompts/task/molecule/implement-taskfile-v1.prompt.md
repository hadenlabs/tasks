# Prompt: Actualizar Taskfile template de molecule

## Contexto

Estas trabajando dentro de este repositorio. Debes seguir estrictamente todas las reglas definidas en `AGENTS.md`.

Este repo es una coleccion de templates `Taskfile.yml` (go-task) + automatizacion del proyecto. Debes mantener el template en `src/molecule/Taskfile.yml` y su documentacion de uso remoto.
...
Actualizar y estandarizar el template de Taskfile para la herramienta: `molecule`
...
- Estilo Taskfile:
  - Usar `version: "3"`.
  - Usar `---` al inicio cuando aplique; para `src/molecule/Taskfile.yml`, incluyelo.
...
1. Crear o actualizar `src/molecule/Taskfile.yml`
   - Debe seguir patrones de templates existentes.
   - Incluir como minimo:
     - `check` (orquesta dependencias de checks)
     - `check:molecule` (o `check:<binary>` real) con `preconditions: command -v ...`
   - Incluir tareas adicionales solo si existe un equivalente claro para molecule.
   - No introducir un build system nuevo.

2. Registrar (si aplica) el template en el root `Taskfile.yml`
   - Agregar el include:
     - `molecule: ./src/molecule/Taskfile.yml`
   - Solo si el Taskfile requiere version parametrizable, agregar en `vars:` del root:
     - `MOLECULE_VERSION: <valor>`

3. Documentar el include remoto en `docs/usage.md`
   - Agregar una entrada en el bloque `includes:` con el formato existente:
     - `molecule:`
       - `taskfile: "https://raw.githubusercontent.com/hadenlabs/tasks/refs/heads/main/src/molecule/Taskfile.yml"`
   - Si agregaste `MOLECULE_VERSION` (u otras vars), reflejarlo tambien en el bloque `vars:` del ejemplo (solo si aplica).
...
1. Implementacion completa de `src/molecule/Taskfile.yml`
