# Prompt: Actualizar Taskfile template de python

## Contexto

Estas trabajando dentro de este repositorio. Debes seguir estrictamente todas las reglas definidas en `AGENTS.md`.

Este repo es una coleccion de templates `Taskfile.yml` (go-task) + automatizacion del proyecto. Debes mantener el template en `src/python/Taskfile.yml` y su documentacion de uso remoto.
...
Actualizar y estandarizar el template de Taskfile para la herramienta: `python`
...
- Estilo Taskfile:
  - Usar `version: "3"`.
  - Usar `---` al inicio cuando aplique; para `src/python/Taskfile.yml`, incluyelo.
...
1. Crear o actualizar `src/python/Taskfile.yml`
   - Debe seguir patrones de templates existentes.
   - Incluir como minimo:
     - `check` (orquesta dependencias de checks)
     - `check:python` (o `check:<binary>` real) con `preconditions: command -v ...`
   - Incluir tareas adicionales solo si existe un equivalente claro para python.
...
3. Documentar el include remoto en `docs/usage.md`
   ...
   - Si agregaste `PYTHON_VERSION` (u otras vars), reflejarlo tambien en el bloque `vars:` del ejemplo (solo si aplica).
...
1. Implementacion completa de `src/python/Taskfile.yml`
