# Prompt: Actualizar Taskfile template de yarn

## Contexto

Estas trabajando dentro de este repositorio. Debes seguir estrictamente todas las reglas definidas en `AGENTS.md`.

Este repo es una coleccion de templates `Taskfile.yml` (go-task) + automatizacion del proyecto. Debes mantener el template en `src/yarn/Taskfile.yml` y su documentacion de uso remoto.
...
Actualizar y estandarizar el template de Taskfile para la herramienta: `yarn`
...
- Estilo Taskfile:
  - Usar `version: "3"`.
  - Usar `---` al inicio cuando aplique; para `src/yarn/Taskfile.yml`, incluyelo.
...
1. Crear o actualizar `src/yarn/Taskfile.yml`
   - Debe seguir patrones de templates existentes.
   - Incluir como minimo:
     - `check` (orquesta dependencias de checks)
     - `check:yarn` (o `check:<binary>` real) con `preconditions: command -v ...`
   - Incluir tareas adicionales solo si existe un equivalente claro para yarn.
   - No introducir un build system nuevo.

2. Registrar (si aplica) el template en el root `Taskfile.yml`
   - Agregar el include:
     - `yarn: ./src/yarn/Taskfile.yml`
...
3. Documentar el include remoto en `docs/usage.md`
   - Agregar una entrada en el bloque `includes:` con el formato existente:
     - `yarn:`
       - `taskfile: "https://raw.githubusercontent.com/hadenlabs/tasks/refs/heads/main/src/yarn/Taskfile.yml"`
   - Si agregaste `YARN_VERSION` (u otras vars), reflejarlo tambien en el bloque `vars:` del ejemplo (solo si aplica).
...
1. Implementacion completa de `src/yarn/Taskfile.yml`
